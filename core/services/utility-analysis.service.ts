...
export class UtilityAnalysisService {
   ...
   */
  public analyzeUtilityFunctions(inputData: AnalysisData): UtilityDetectionResult {
    ...
    const functionAnalyses: FunctionAnalysis[] = codeNodes.map(node => 
      this.analyzeSingleFunction(node)
    );
   ...
   */
  private analyzeSingleFunction(node: CodeNode): FunctionAnalysis {
   ...
   */
  private extractFeatures(code: string): FunctionFeatures {
   ...
   */
  private countExternalCalls(code: string): number {
    const patterns = [
      /\w+\.\w+\(/g, // method calls
      /\w+\(/g // function calls
    ];
   ...
   */
  private hasDocstring(code: string): boolean {
   ...
   */
  private hasDecorators(code: string): boolean {
   ...
   */
  private countStringOperations(code: string): number {
    const patterns = [
   ...
   */
  private countMathOperations(code: string): number {
    const patterns = [
   ...
   */
  private countConditionalStatements(code: string): number {
   ...
   */
  private countMultiplePatterns(code: string, patterns: RegExp[]): number {
    let count = 0;
    patterns.forEach(pattern => {
      const matches = code.match(pattern);
      if (matches) count += matches.length;
   ...
   */
  private calculateUtilityScore(label: string, code: string, features: FunctionFeatures): number {
    let score = 0;

    // Name-based scoring
    const labelLower = label.toLowerCase();
    const hasUtilityKeyword = this.UTILITY_KEYWORDS.some(keyword => 
      labelLower.includes(keyword)
    );
    if (hasUtilityKeyword) score += 0.3;

    // Simple function characteristics
    if (features.linesOfCode <= 5) score += 0.2;
    if (features.cyclomaticComplexity <= 2) score += 0.15;
    if (features.nestingLevel <= 1) score += 0.1;
    if (features.parameterCount <= 2) score += 0.1;
    if (features.returnStatements === 1) score += 0.05;

    // String/math operations indicate utility functions
    if (features.stringOperations > 0) score += Math.min(0.1, features.stringOperations * 0.02);
    if (features.mathOperations > 0) score += Math.min(0.1, features.mathOperations * 0.02);

    // Path-based scoring (check if in utils/helpers directory)
    if (/utils?|helpers?|lib|common/i.test(code)) score += 0.1;
   ...
   */
  private calculateBusinessRelevanceScore(label: string, code: string, features: FunctionFeatures): number {
    let score = 0;

    // Name-based scoring
    const labelLower = label.toLowerCase();
    const hasBusinessKeyword = this.BUSINESS_KEYWORDS.some(keyword => 
      labelLower.includes(keyword)
    );
    if (hasBusinessKeyword) score += 0.3;

    // FastAPI-specific patterns
    const hasFastAPIDecorator = this.FASTAPI_DECORATORS.some(decorator => 
      code.includes(decorator)
    );
    if (hasFastAPIDecorator) score += 0.4;

    // API endpoint patterns
    if (/route|endpoint|api/i.test(labelLower)) score += 0.2;
    if (/(get|post|put|delete|patch)_/i.test(labelLower)) score += 0.2;

    // Complex business logic indicators
    if (features.cyclomaticComplexity > 5) score += 0.1;
    if (features.linesOfCode > 20) score += 0.1;
    if (features.nestingLevel > 2) score += 0.1;

    // Database/external service interactions
    if (/database|db|query|session|repository|service/i.test(code)) score += 0.15;
    if (/async|await/i.test(code)) score += 0.1;
   ...
   */
  private calculateComplexityScore(features: FunctionFeatures): number {
    let score = 0;
  ...
  private classifyFunction(
    ...
    businessRelevanceScore: number, 
    utilityScore: number
  ): 'business-logic' | 'utility' | 'infrastructure' {
   ...
   */
  private generateSummary(analyses: FunctionAnalysis[]) {
   ...
   */
  public getTopImportantFunctions(result: UtilityDetectionResult, n: number): FunctionAnalysis[] {
   ...
   */
  public getBottomUtilityFunctions(result: UtilityDetectionResult, n: number): FunctionAnalysis[] {
