...
export function layoutGraph(
  ...
  // 2. Assign layers using longest path layering (topological) with enforced tiers
  const layers = new Map<string, number>();
  // Enforce hierarchy baselines: C1 at 0, C2 at 1, graph nodes start at 2
  const baseLayer = new Map<string, number>();
  c1Outputs.forEach(n => baseLayer.set(n.id, 0));
  c2Subcategories.forEach(n => baseLayer.set(n.id, 1));
  graphNodes.forEach(n => baseLayer.set(n.id, 2));
  ...
  function dfs(node: string): number {
    ...
    let layer = baseLayer.get(node) ?? 0;
  ...
  const NODE_SPACING = 500; // Minimum horizontal space between nodes in the same layer
  const STAGGER = 100; // Horizontal stagger for alternating nodes in a layer
